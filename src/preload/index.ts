...
const api = {
  ...
  users: {
    ...
    assignStore: (userId, storeId, role) => ipcRenderer.invoke('users:assignStore', { userId, storeId, role }),
    removeStore: (userId, storeId) => ipcRenderer.invoke('users:removeStore', { userId, storeId }),
    updateStoreRole: (userId, storeId, role) => ipcRenderer.invoke('users:updateStoreRole', { userId, storeId, role }),
  },
  roles: {
  ...
  profile: {
    ...
    changePassword: (id, currentPassword, newPassword) => ipcRenderer.invoke('profile:changePassword', { id, currentPassword, newPassword }),
  },
  app: {
    uploadImage: (data: { base64Data: string, fileName: string }) => ipcRenderer.invoke('app:uploadImage', data)
  },
  sales: {
