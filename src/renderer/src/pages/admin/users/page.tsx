...
export default function UsersPage() {
  ...
  const onSubmit: SubmitHandler<UserFormData> = async (values) => {
    ...
    try {
      if (editingUserId) {
        const result = await window.api.users.update(editingUserId, values);
        if (result.success) {
            ...
            <div className="p-4 rounded-lg bg-muted/50 border border-border space-y-4">
              ...
              <div className="flex gap-3">
                <div className="flex-1">
                  <Select
                    value={assignStoreId}
                    onValueChange={setAssignStoreId}
                  >
                    ...
                    <SelectContent>
                      {stores
                        .filter(
                          (s: any) =>
                            !userStores.find((us: any) => us.store?._id === s._id)
                        )
                        .map((store: any) => (
                      ...
                      >
                        ...
                        <div className="flex items-center gap-2">
                          <Select
                            ...
                            onValueChange={(val) =>
                              item.store?._id &&
                              handleUpdateRole(item.store._id, val)
                            }
                            disabled={!item.store}
                          >
                          ...
                          <Button
                            ...
                            onClick={() =>
                              handleRemoveAccess(item.store?._id || item._id)
                            }
                          >
