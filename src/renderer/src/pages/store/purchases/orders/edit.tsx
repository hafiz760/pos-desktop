...
import { Plus, Search, X, PlusCircle, ArrowLeft, UserPlus, Save, Truck } from "lucide-react";
                                    ...
                                    <Input
                                        ...
                                        onKeyDown={(e) => {
                                            if (e.key === "ArrowDown") {
                                                ...
                                                if (
                                                    showAutocomplete &&
                                                    filteredProducts[selectedSuggestionIndex]
                                                ) {
                                                    addProductToOrder(
                                                        filteredProducts[selectedSuggestionIndex]
                                                    );
                                        ...
                                        filteredProducts.length > 0 && (
                                            <div className="absolute z-50 w-full mt-1 bg-popover border border-border rounded-md shadow-2xl max-h-[400px] overflow-y-auto">
                                                {filteredProducts.map((p: any, idx: number) => (
                                                    <div
                                                        key={p._id}
                                                        className={`p-3 cursor-pointer flex items-center justify-between border-b border-border last:border-0 ${idx === selectedSuggestionIndex
                                    ...
                                    <table className="w-full text-sm text-left border-collapse">
                                        <thead className="bg-muted text-muted-foreground text-[10px] uppercase font-black sticky top-0 z-10 backdrop-blur-sm">
                                            <tr>
                                                ...
                                                <th className="px-4 py-3 border-r border-border text-center w-24">
                                                    Qty
                                                </th>
                                ...
                                <div className="divide-y divide-border/50">
                                    {products
                                        ...
                                        .map((product: any) => (
                                            <div
                                                key={product._id}
                                                className={`grid grid-cols-12 gap-2 p-3 items-center transition-colors cursor-pointer group ${selectedBulkProducts.includes(product._id)
                                                    ? "bg-[#4ade80]/10"
                                                    : "hover:bg-accent/50"
                                                    }`}
                                                onClick={() => {
