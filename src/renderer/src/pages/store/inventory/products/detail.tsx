        ...
        <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
            <div className="flex items-center justify-between">
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        ...
                        <div className="flex items-center gap-2 text-muted-foreground text-sm">
                            <Badge variant="outline" className="border-border text-[10px] uppercase text-muted-foreground">{product.category?.name}</Badge>
                            <span>•</span>
                ...
                <div className="flex gap-3">
                    <Button variant="outline" className="border-border hover:bg-accent text-foreground" onClick={() => navigate(`/dashboard/inventory/products?edit=${product._id}`)}>
            ...
            <div className="grid gap-6 md:grid-cols-3">
                ...
                <Card className="bg-card border-border text-foreground">
                    ...
                    <CardContent>
                        <div className="text-3xl font-bold text-[#4ade80]">Rs. {(product.sellingPrice || 0).toLocaleString()}</div>
                ...
                <Card className="bg-card border-border text-foreground">
                    ...
                    <CardContent>
                        <div className="text-3xl font-bold text-blue-400">Rs. {(product.buyingPrice || 0).toLocaleString()}</div>
                        <p className="text-xs text-muted-foreground mt-1">Last Purchase Unit Cost</p>
                    </CardContent>
                </Card>

                <Card className={`bg-card border-border text-foreground ${(product.stockLevel || 0) < (product.minStockLevel || 5) ? 'border-red-500/50 ring-1 ring-red-500/20' : ''}`}>
                    <CardHeader className="pb-2">
                        <CardTitle className="text-xs font-medium text-muted-foreground uppercase flex items-center gap-2">
                            ...
                            <CardContent className="p-0">
                                <div className="grid md:grid-cols-2">
                                    ...
                                    <div className="p-6 space-y-4 bg-card/50">
                                        <h3 className="text-sm font-bold uppercase tracking-wider text-muted-foreground">Product Gallery</h3>
                        ...
                        <CardContent className="p-0">
                            <div className="divide-y divide-border">
                                <div className="grid grid-cols-3 p-4">
                                    ...
                                    <span className="col-span-2 text-sm text-foreground">{product.brand?.name || "No Brand"}</span>
                                </div>
                                <div className="grid grid-cols-3 p-4">
                                    <span className="text-muted-foreground text-sm font-medium">Model / SKU</span>
                                    <span className="col-span-2 text-sm text-foreground font-mono uppercase">{product.sku}</span>
                                </div>
                                <div className="grid grid-cols-3 p-4">
                                ...
                                {product.barcode && (
                                    <div className="grid grid-cols-3 p-4">
                                        ...
                                        <span className="col-span-2 text-sm text-foreground font-mono">{product.barcode}</span>
                                    </div>
                                )}
                                <div className="grid grid-cols-3 p-4">
                                    <span className="text-muted-foreground text-sm font-medium">Minimum Stock Alert</span>
                                    <span className="col-span-2 text-sm text-foreground">{product.minStockLevel || 5} items</span>
                                </div>
                            </div>
                        </CardContent>
                    </Card>
