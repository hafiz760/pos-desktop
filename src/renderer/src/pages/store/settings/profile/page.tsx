    ...
    useEffect(() => {
        const loadUserAndRoles = async () => {
            ...
            if (userStr) {
                ...
                setSelectedRole(user.role || user.globalRole || "USER");
            }

            try {
                const rolesRes = await window.api.roles.getAll();
                if (rolesRes.success) {
        ...
        try {
            ...
            if (avatarInput?.files?.[0]) {
                ...
                if (uploadResult.success) {
                    updateData.avatarUrl = uploadResult.data;
                } else {
            ...
            if (result.success) {
                ...
                // No need to refresh page, state update handles it
            } else {
        ...
        <div className="space-y-6 animate-in fade-in duration-500">
            <div className="flex items-center justify-between mb-6">
                <div className="flex items-center gap-4">
                    <Button variant="ghost" size="icon" onClick={() => navigate(-1)} className="border border-border">
                        <ArrowLeft className="h-5 w-5" />
                    </Button>
                    <div>
                        <h2 className="text-3xl font-black tracking-tight text-foreground uppercase">My Profile</h2>
                        <p className="text-muted-foreground text-xs font-bold uppercase opacity-70">Manage your account settings and preferences.</p>
                    </div>
                </div>
            </div>
                ...
                <CardHeader className="bg-muted/30 border-b border-border">
                    <div className="flex items-center gap-4">
                        ...
                        <div>
                            ...
                            <CardDescription className="text-xs font-bold uppercase opacity-70">Update your photo and personal details here.</CardDescription>
                        </div>
                    </div>
                </CardHeader>
                        ...
                        <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-8">
                            <div className="flex flex-col md:flex-row items-center md:items-start gap-8 py-4">
                                ...
                                <div className="space-y-4 flex-1 w-full text-center md:text-left">
                                    <div className="space-y-1">
                                        <Label htmlFor="avatar" className="font-black uppercase text-sm">Profile Photo</Label>
                                        <p className="text-[10px] text-muted-foreground font-bold uppercase opacity-70">Recommended: Square JPG, PNG. Max 2MB.</p>
                                    </div>
                                    <div className="flex justify-center md:justify-start">
                                        <Label htmlFor="avatar-upload" className="cursor-pointer inline-flex items-center justify-center rounded-md text-xs font-black uppercase transition-all focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-ring disabled:pointer-events-none disabled:opacity-50 border border-input bg-background shadow-sm hover:bg-[#4ade80] hover:text-black hover:border-[#4ade80] h-10 px-6 py-2 gap-2">
                            ...
                            <div className="grid gap-6 md:grid-cols-2">
                                ...
                                <div className="space-y-2">
                                    <Label htmlFor="email" className="text-[10px] font-black uppercase text-muted-foreground">Email Address</Label>
                                    <Input
                                        id="email"
                                        ...
                                        disabled
                                    />
                                    <p className="text-[10px] text-muted-foreground font-bold uppercase opacity-50">Email cannot be changed</p>
                                </div>

                                <div className="space-y-2">
                                    <Label htmlFor="role" className="text-[10px] font-black uppercase text-muted-foreground">Role</Label>
                                    <Input
                                        id="role"
                                        defaultValue={currentUser?.role || currentUser?.globalRole || "USER"}
                                        className="bg-muted/50 border-border text-foreground font-bold h-11 opacity-70"
                                        disabled
                                    />
                                    <p className="text-[10px] text-muted-foreground font-bold uppercase opacity-50">Role is managed by admin</p>
                                </div>
                                ...
                                <FormField
                                    ...
                                    render={({ field }) => (
                                        <FormItem className="space-y-2 md:col-span-2">
                                            <FormLabel className="text-[10px] font-black uppercase text-muted-foreground">Change Password</FormLabel>
                                            <FormControl>
                                                <Input
                                                    {...field}
                                                    type="password"
                                            ...
                                            <FormMessage className="text-[10px] font-bold uppercase" />
                                            <p className="text-[10px] text-muted-foreground font-bold uppercase opacity-70">Leave blank if you don't want to change your password.</p>
                                        </FormItem>
                                    )}
                                />
                            </div>
